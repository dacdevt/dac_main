...
import (
	...
	"github.com/dacchain/dacapp/dac/downloader"
	"github.com/dacchain/dacapp/dac/fetcher"
	...
	"github.com/dacchain/dacapp/p2p/discover"
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing dacChain peer", "peer", id)

	// Unregister the peer from the downloader and dacChain peer set
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("dacChain peer connected", "name", p.Name())

	// Execute the dacChain handshake
	td, head, genesis := pm.blockchain.Status()
	if err := p.Handshake(pm.networkId, td, head, genesis); err != nil {
		p.Log().Debug("dacChain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("dacChain peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("dacChain message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= dac1 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= dac1 && msg.Code == NodeDataMsg:
	...
	case p.version >= dac1 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= dac1 && msg.Code == ReceiptsMsg:
...
// NodeInfo represents a short summary of the dacChain sub-protocol metadata
