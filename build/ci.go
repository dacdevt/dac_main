...
func Env() Environment {
	env := applyEnvFlags(Environment{Name: "local", Repo: "dacchain/dacapp"})
...
func doInstall(cmdline []string) {
	...
	if !strings.Contains(runtime.Version(), "devel") {
		...
		if minor < 7 {
			...
			log.Println("dacapp requires at least Go version 1.7 and cannot")
	...
	if *arch == "" || *arch == runtime.GOARCH {
		...
		goinstall.Args = append(goinstall.Args, []string{"-o", executablePath("dacapp")}...)
